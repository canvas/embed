...
export const Chart = React.forwardRef<ChartHandle, ChartProps>(function Chart(props: ChartProps, ref) {
...
function guessPrecisionAndFormatNumber(value: number, comparableValue: number): string {
...
function getOptions(
    ...
    let chartType: 'line' | 'bar' | 'column' | 'funnel' | 'pie' | 'scatter' | 'area' = 'line';
    ...
    return {
        ...
        series: data.values.map((series: string[], seriesIndex: number) => {
            ...
            if (chartType === 'pie') {
                ...
                seriesData = series.map((x: string, index: number) => [domainLabels[index], parseFloat(x)]);
...
function getTopChartValues(chartData: ChartData) {
    ...
    const maxValues = 12;
    // exclude values <1%
    const significantCount = mapped.slice(0, maxValues).reduce((s, a) => s + (a[1] / total >= 0.01 ? 1 : 0), 0);
