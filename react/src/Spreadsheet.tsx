...
function parseDateTimeNtz(value: string): DateTime {
  ...
  const bigQueryMillisParse = DateTime.fromFormat(
    value,
...
export function parseDateTimeTz(value: string): DateTime {
  ...
  const bigQueryParse = DateTime.fromFormat(
    value,
...
function getCopyData(
  ...
  switch (selectionArea.type) {
    case "cells":
      ...
      return data;
    case "columns":
      const { startColumn: cStartColumn, endColumn: cEndColumn } =
        selectionArea;
      const cData: string[][] = [];
      for (let rowIndex = -1; rowIndex < dataStore.rowCount; rowIndex++) {
        const row: string[] = [];
        for (
          let columnIndex = cStartColumn;
          columnIndex <= cEndColumn;
          columnIndex++
        ) {
          if (rowIndex === -1) {
            row.push(headers[columnIndex]!);
    ...
    case "rows":
      ...
      for (let rowIndex = rStartRow; rowIndex <= rEndRow; rowIndex++) {
        ...
        for (
          let columnIndex = 0;
          columnIndex < dataStore.columnCount;
          columnIndex++
        ) {
          if (rowIndex === -1) {
            row.push(headers[columnIndex]!);
...
const HeaderCell = ({
  ...
  const selectedStyle = cellSelected(
    columnIndex,
    -1,
    selectionArea,
    columnCount,
  ...
  const lastColumnStyle =
    spreadsheetSize.fullWidth && columnIndex == columnCount - 1
      ? { borderRight: "none" }
      : {};
...
const containsRow = (
  rowIndex: number,
  selectionArea: RowSelectionArea,
): Contains | undefined => {
...
const containsColumn = (
  columnIndex: number,
  selectionArea: ColumnSelectionArea,
): Contains | undefined => {
  if (
    columnIndex >= selectionArea.startColumn &&
    columnIndex <= selectionArea.endColumn
  ) {
    return {
...
const cellSelected = (
  ...
  switch (selectionArea.type) {
    case "cells":
      const [row, column] = [
        containsRow(rowIndex, selectionArea),
        containsColumn(columnIndex, selectionArea),
      ];
      if (row && column) {
        return {
    ...
    case "columns":
      ...
      if (cColumn) {
        return {
          ...
          borderBottom:
            rowIndex === rowCount - 1 ? SELECTED_CELL_BORDER : CELL_BORDER,
...
const DataCell = memo(
  ({ columnIndex, rowIndex, style, data: itemData }: CellProps) => {
    const spreadsheetSize = useContext(SpreadsheetSizeContext);
    const {
      dataStore,
      onMouseDown,
    ...
    const lastRowStyle =
      spreadsheetSize.fullHeight && rowIndex == rowCount - 1
        ? { borderBottom: "none" }
        : {};
    const lastColumnStyle =
      spreadsheetSize.fullWidth && columnIndex == columnCount - 1
    ...
    const bgClass = backgroundClass(
      conditionalFormatting?.get(`${columnIndex}/${rowIndex}`) ?? null,
    );
    return (
      <div
...
const TopLeft = ({
  ...
  style,
  data: itemData,
}: GridChildComponentProps<TopLeftItemData>) => {
...
const ColumnGutterCell = ({
  columnIndex,
  rowIndex,
  style,
  data,
}: GridChildComponentProps<ColumnGutterCellData>) => {
  const spreadsheetSize = useContext(SpreadsheetSizeContext);
  const {
    onMouseDown,
    onMouseEnter,
...
const RowGutterCell = ({
  ...
  const spreadsheetSize = useContext(SpreadsheetSizeContext);
  const { onMouseDown, onMouseEnter, onMouseUp, selectionArea, rowCount } =
    data;
...
const getColumnIdsForRange = (
  ...
  return [...Array(higher - lower).keys()].map(
    (index) => metaData.visibleColumns[index + lower]!,
  );
};
...
const getColumnArea = (
  event1: ColumnLocation,
  event2: ColumnLocation,
): ColumnSelectionArea => {
  return {
...
export function SpreadsheetTable({
  ...
  const { rowCount, columnCount, data, metaData, storeId, dataHash } =
    dataStore;
  ...
  const dataWidths = metaData.visibleColumns.map((_, index) => {
    ...
    const valueLength = Math.max(
      0,
      ...data[index]!.slice(0, 50).map(
        (v) => formatCell(v, format, type, timezone).length,
      ),
    );
  ...
  React.useEffect(() => {
    const unchanged =
      _.isEqual(dataWidths, prevMeta?.dataWidths) &&
      _.isEqual(dataHash, prevMeta?.dataHash);
    if (unchanged) {
      return;
    }
    resetWidths(0);
  ...
  const [mouseDownPoint, setMouseDownPoint] =
    React.useState<MouseLocation | null>(null);
  ...
  const mouseMove = React.useCallback(
    ...
    [
      mouseDownPoint,
      setCellSelectionArea,
      setColumnSelectionArea,
      setRowSelectionArea,
    ],
  );
  const dataCellItemData: DataCellItemData = React.useMemo(
    () => ({
      ...
      selectionArea,
      onMouseDown: (column: number, row: number) =>
        mouseDown({ type: "cells", column, row }),
      onMouseEnter: (column: number, row: number) =>
        mouseMove({ type: "cells", column, row }),
      onMouseUp: mouseUp,
      columnCount,
      rowCount,
      conditionalFormatting,
  ...
  const headerItemData: HeaderCellItemData = {
    ...
    rowCount,
    onMouseUp: () => mouseUp(),
    onMouseDown: (column: number) =>
      mouseDown({ type: "cells", column, row: -1 }),
    onMouseEnter: (column: number) =>
      mouseMove({ type: "cells", column, row: -1 }),
    columnTypes,
  };
  const columnGutterItemData: ColumnGutterCellData = React.useMemo(
    () => ({
      onMouseDown: (column: number) =>
        mouseDown({ type: "column_gutter", column }),
      onMouseEnter: (column: number) =>
        mouseMove({ type: "column_gutter", column }),
      onMouseUp: mouseUp,
      onContextMenu: (
        event: React.MouseEvent<HTMLDivElement>,
        columnIndex: number,
      ...
      switch (selectionArea.type) {
        case "cells":
          ...
          if (
            ...
            const { startColumn, endColumn, startRow, columnIds } =
              selectionArea;
  ...
  return (
    <div
      className="flex border-collapse flex-col rounded-[5px] border border-divider"
...
export const EmptySpreadsheetGrid = (): React.ReactElement => {
  ...
  const itemData: ColumnGutterCellData &
    HeaderCellItemData &
    RowGutterCellData &
    DataCellItemData &
    TopLeftItemData = {
...
const InnerSpreadsheetGrid = React.forwardRef<
  InnerSpreadsheetGridHandle,
...
>(
  (
    ...
    const headerOnScroll: (props: GridOnScrollProps) => void =
      React.useCallback(
    ...
    const columnGutterOnScroll: (props: GridOnScrollProps) => void =
      React.useCallback(
        ({ scrollLeft, scrollUpdateWasRequested }) => {
          if (!scrollUpdateWasRequested) {
            headerGrid.current?.scrollTo({ scrollLeft });
    ...
    const rowGutterOnScroll: (props: GridOnScrollProps) => void =
      React.useCallback(
