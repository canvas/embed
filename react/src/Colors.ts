...
import { ColorTheme } from './__rust_generated__/ColorTheme';
...
function colorPalette(colorTheme: ColorTheme): ColorPalette {
    switch (colorTheme) {
        case 'Bright':
            return {
                type: 'categorical',
                preview: ['#5ba8f7', '#9b54f3', '#72b622', '#e6482c', '#f98517', '#fdd146'],
        ...
        case 'Rainbow':
            return {
                type: 'categorical',
                preview: ['#3742A0', '#129DC9', '#28C093', '#DF3F3F', '#F48D54', '#FAC372'],
...
function colorTable(colorTheme: ColorTheme, steps: number, preview?: boolean, dark?: boolean): string[] {
    ...
    if (palette.type == 'scale') {
        ...
        if (dark) {
            return palette.darkColors ? palette.darkColors.slice(0, steps) : palette.colors.slice(0, steps);
        }
